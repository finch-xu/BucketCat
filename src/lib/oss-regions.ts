...
export function findOssRegion(id: string): OssRegion | undefined {
  return OSS_REGIONS.find((r) => r.id === id);
}
...
/** Looks up which region an endpoint belongs to (public or internal
 * endpoint, with or without an `https://` prefix). Returns undefined for
 * custom domains that aren't in the table. */
export function ossRegionFromEndpoint(endpoint: string): OssRegion | undefined {
  const bare = stripScheme(endpoint).toLowerCase();
  if (!bare) return undefined;
  return OSS_REGIONS.find(
    (r) => r.endpoint.toLowerCase() === bare || r.internal.toLowerCase() === bare,
  );
}

/** Whether the given endpoint is a known region's internal (VPC) endpoint. */
export function isInternalOssEndpoint(endpoint: string): boolean {
  const bare = stripScheme(endpoint).toLowerCase();
  if (!bare) return false;
  return OSS_REGIONS.some((r) => r.internal.toLowerCase() === bare);
}

/** Assembles the final endpoint value to put in the form, with an
 * `https://` prefix, always by table lookup (never string concatenation). */
